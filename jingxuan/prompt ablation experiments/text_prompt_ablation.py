...
def save_cv2_eval(prompt,inference_step_star,manipulated_mask_func,reverse_mask,one_step_mode,avg_scores,mask_kwargs):
...
def generate_and_quantify_images(save_dir,axes, pipeline, prompt, max_length=120, weight_dtype=torch.bfloat16,
                   num_inference_steps=14, guidance_scale=4.5, num_images_per_prompt=25, device="cuda", random_seed=0,inference_step_star=None,manipulated_mask_func=None,reverse_mask=False,one_step_mode=False,**mask_kwargs):
    image_logs, latents_traj, pred_traj, t_traj, denoiser_traj = visualize_prompt_with_traj(pipeline=pipeline, prompt=prompt, max_length=max_length, weight_dtype=weight_dtype, num_images_per_prompt=num_images_per_prompt, inference_step_star=inference_step_star, manipulated_mask_func=manipulated_mask_func,reverse_mask=reverse_mask,one_step_mode=one_step_mode,**mask_kwargs)
    ...
    save_cv2_eval(prompt,inference_step_star,manipulated_mask_func,reverse_mask,one_step_mode,avg_scores,mask_kwargs)
...
def generate_and_quantify_images_batch(save_dir, pipeline, prompt, prompt_idx, max_length, weight_dtype, mask_func, reverse_mask, one_step_mode,mask_type):
    fig, axeses = plt.subplots(14,14, figsize=(14,14))
    for j, axes in enumerate(axeses):
        generate_and_quantify_images(save_dir, axes, pipeline, prompt, max_length=max_length, weight_dtype=weight_dtype,
            ...
            one_step_mode=one_step_mode, parts_to_mask=mask_type)
    plt.savefig(os.path.join(save_dir, f'prompt_{prompt_idx}_mask_{mask_type}.png'))
    plt.close()
...
def main():
    ...
    parser.add_argument('--mask_func', type=str, default='mask_semantic_parts_attention', help='Mask function to use')
    ...
    for i, prompt in enumerate(prompts):
        ...
        for p in parts_to_mask:
            generate_and_quantify_images_batch(save_dir, pipeline, prompt, i, p, max_length)
