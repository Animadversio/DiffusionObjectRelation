...
def extract_caption_t5(max_tokens=120):
    global t5
    global t5_save_dir
    # global images_extension
    t5 = T5Embedder(device="cuda", local_cache=True, cache_dir=f'{args.pretrained_models_dir}/t5_ckpts', model_max_length=max_tokens)
...
def extract_img_vae():
    ...
    for image_name in tqdm(lines):
        ...
        if os.path.exists(f"{save_path}.npy"):
            continue
        try:
            img = Image.open(f'{args.dataset_root}/images/{image_name}')
...
def get_args():
    ...
    parser.add_argument("--max_tokens", default=120, type=int, help="max tokens for caption t5 feature extraction")
...
if __name__ == '__main__':
    ...
    extract_caption_t5(args.max_tokens)
