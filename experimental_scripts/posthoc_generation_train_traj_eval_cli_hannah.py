...
def eval_train_traj(savedir, pipeline):
    eval_df_all_traj = []
    object_df_all_traj = []
    # step_num = 160000
    ckptdir = join(savedir, "checkpoints")
...
def plot_and_save_eval_train_traj(eval_df_all_traj, model_run_name, synopsis_dir, poc="hk"):
    # Save the images 
    ...
    saveallforms(synopsis_dir, f"{poc}_{model_run_name}_eval_train_dynamics_traj_syn")
    plt.show()


def plot_and_save_relation_eval_heatmap(eval_df_all, model_run_name, values, poc="hk"):
    '''
    Some examples of values:
        'spatial_relationship_acc'
        'shape_acc'
        'color_acc'
        'overall_acc'
        'Dx'
    ...
    eval_syn_df = eval_syn_df.rename(columns={'overall':'overall_acc', 
                                              'shape':'shape_acc', 
                                              'color': 'color_acc', 
                                              'spatial_relationship':'spatial_relationship_acc'})
    ...
    sns.heatmap(pivot_df,
                ...
                cmap='viridis', 
                fmt='.2f', 
                cbar_kws={'label': f'{values} Accuracy'},
                )

    plt.title(f'{values} by Object Attributes (mean of 100 samples per prompt)\n{model_run_name}')
    plt.xlabel('Prompt Spatial Relationship')
    plt.ylabel('Object Attributes (color1, shape1, color2, shape2)')
    plt.xticks(rotation=45)
    plt.yticks(rotation=0)
    plt.tight_layout()
    saveallforms(eval_dir, f"relation_eval_heatmap_{values}_acc_{model_run_name}")
    plt.show()
...
if __name__ == "__main__":
    ...
    for model_run_name in model_run_names:
        ...
        figdir = f"/n/holylfs06/LABS/kempner_fellow_binxuwang/Users/binxuwang/DL_Projects/PixArt/{suffix}_{model_run_name}/cross_attn_vis_figs"
        # result_dir = f"/n/holylfs06/LABS/kempner_fellow_binxuwang/Users/binxuwang/DL_Projects/PixArt/analysis_results/{model_run_name}_{suffix}"
        synopsis_dir = f"/n/home12/hjkim/Github/DiffusionObjectRelation/Figures/model_eval_synopsis"
        ...
        print(f"processing {model_run_name} ...")
        ...
        eval_df_all_traj, object_df_all_traj = eval_train_traj(savedir, pipeline)

        print(f"plotting {model_run_name} ...")
        plot_and_save_eval_train_traj(eval_df_all_traj, model_run_name, synopsis_dir)
        # plot_and_save_relation_eval_heatmap(eval_df_all_traj, model_run_name, "spatial_relationship_acc")
